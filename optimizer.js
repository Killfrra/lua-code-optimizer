...
class Var {
    ...
    opt_candidates = [];
    constructor(name, value, assigner) {
        this.name = name;
...
class Scope {
    ...
    parent_scope;
    isolate = true;
    constructor(block, parent_scope) {
    ...
    remove(assigner, name) {
        if (!assigner) {
            return;
        }
        else if (assigner.type === 'FunctionDeclaration' || (assigner.type === 'AssignmentStatement' &&
    ...
    setUndefined(name, opt_candidates, opt_candidate) {
...
function walk(node, scope, fn) {
    if (fn(node, scope) === false)
        return;
    for (let [key, child] of Object.entries(node)) {
        if (child === null || typeof child !== 'object') {
            continue;
        ...
        if (key === 'body') { //TODO: fix any
            sub_scope = new Scope(node, scope);
        }
        if (Array.isArray(child)) {
            for (let sub of child) {
